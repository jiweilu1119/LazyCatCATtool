...
?>
	...
	<div class="container " style="width: 100%">
		<div class="row clearfix">
		<p><label for="inputEmail3" class="col-sm-8 control-label"><?php echo $lang["查找术语"]; ?></label></p>
		</div>
	<div class="row clearfix">
	
	
	<form class="form-horizontal" role="form" action="term.php" method="POST">
...
<!--old version		<div class="container " style="width: 80%">
	<div class="row clearfix"><form class="form-horizontal" role="form" action="term.php" method="POST">
		<div class="col-md-10 column">
			
				<div class="form-group" >
					 <label for="inputEmail3" class="col-sm-2 control-label"><?php //echo $lang["查找术语"]; ?></label>
					<div class="col-sm-10">
						<input type="text" class="form-control" id="inputEmail3" name="searchkey"/>
					</div></div></div>
					<div class="col-md-2 column" >
...
$searchterm_sql ="SELECT * FROM `tb` WHERE `en_us` LIKE '%$searchkey%' OR `zh_cn` LIKE '%$searchkey%'";
...
echo '<div class="btn-group">
	...
	echo '&way=terms';
	echo '">
	<a href="delete.php?id='.$row["zh_cn"].'&way=terms"><button class="btn btn-default" type="button"><em class="glyphicon glyphicon-align-right"></em> '.$lang["删除"].'</button> </a>
	</div>';
echo "</td></tr>";

}}
else{
	$searchterm_sql ="SELECT * FROM `tb` ";
$searchterm_result = mysqli_query($conn,$searchterm_sql);
...
<?php
	if(isset($_POST['termadd'])){
		$newch = $_POST['newch'];
		$newen = $_POST['newen'];
		$termadd_sql="INSERT INTO `tb` (`zh_cn`, `en_us`) VALUES ('{$newch}', '{$newen}')";
...
?>
<form method="POST" action="term.php" role="form">
<td>
...
<td>
<button type="submit" class="btn btn-default" name="termadd"><?php echo $lang["添加"]; ?></button>
</td>
</form>
</tr>
